...
| `get-teams-chat-message` | Return a single Microsoft Teams chat message by its id via the chat-aggregator. Uses the chatsvcagg-audience bearer captured at login (same identity as the basic Teams token, different audience). **Best-effort, may break on Microsoft client updates** — the chatsvcagg surface is not in the public Microsoft Graph API. Source the chat-id + message-id via `list-teams-chats-with-messages` or `list-teams-chat-messages`. | `--chat-id`, `--message-id` | `GET https://chatsvcagg.teams.microsoft.com/api/v2/users/me/chats/{chat-id}/messages/{message-id}` |
